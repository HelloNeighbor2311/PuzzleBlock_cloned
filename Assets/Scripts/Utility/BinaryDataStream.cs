...
public class BinaryDataStream
{
    public static void Save<T>(T serializedObject, string fileName)
    {
        ...
        FileStream fileStream = new FileStream(Path.Combine(path, fileName + ".dat"),FileMode.Create);
        try
        {
    ...
    public static T Read<T>(string fileName)
    {
        string path = Application.persistentDataPath + "/saves/";
        BinaryFormatter formatter = new BinaryFormatter();
        FileStream fileStream = new FileStream(Path.Combine(path, fileName + ".dat"), FileMode.Open);
